...
You can use the following as a base `Dockerfile` and add packages as needed when we get to different modules of the course:
...
```shell
docker build . -t csosvita:latest
```
...
```shell
docker run --rm -it -v $(pwd):/root csosvita:latest bash
```
